...
六类页面、compiled truth + timeline 模式、`[[slug]]` 引用语法保持不变。Phase 2 不改变 page format，只把 page 内容用于 embedding。
...
这类查询答案在 facts 表里是确定的，向量相似度可能引入不精确页面。`mem ask` 里有一个轻量的 query classifier（纯规则，不调用 LLM）判断是否走 SQL 短路，否则进入三路 RRF。规则无法识别的结构化查询直接走 hybrid fallback。
...
`mem ask --debug` 显示三路各自的 top-10 和 RRF 后的最终排序，便于诊断页面排序原因。
...
- **手动全量**：显式运行 `mem reindex`；可加 `--force` 重新生成所有 embedding（用于更换 model）
- **selective**：`mem reindex --pages <slug>` 重新生成特定页面的 embedding
...
import(path):
    ...
    4. 提示用户运行 mem ingest 处理 laundry
```

**不自动 ingest**：bulk import 把素材放进 laundry 后停止处理，由用户运行 `mem ingest` 单独处理。理由：
- ingest 会调用 LLM，是 import 之外的另一笔成本
- 用户可能想在 ingest 之前预览/筛选 laundry 里的内容
- 错误隔离更清晰：import 出问题就重试 import，ingest 出问题就重试 ingest

如果用户希望一次完成，当前推荐显式串联：`mem import <path> --yes`，然后 `mem ingest`。`mem ingest` 会按配置自动触发增量 reindex；需要跳过时使用 `mem ingest --no-auto-reindex`。
...
class LocalBGEEmbeddingClient: ...  # 预留, Phase 2 不实现
class VoyageEmbeddingClient: ...    # 预留, Phase 2 不实现
```
...
3. **永不静默写入。** Phase 2 的 bulk import 在运行 LLM-backed ingest 之前会停止，让用户检查 laundry；cost estimate 也要显式确认。
4. **Append-only 优先。** Phase 2 的 `embeddings` 表是真正可重建的派生层，content_hash 不匹配就重新生成 embedding，不需要 append-only。
...
- 现有 `mem ask` 在未运行 reindex 时退化成 Phase 1 行为（只 keyword + SQL）
- 升级后需要用户第一次运行 `mem reindex`，把现有 page 索引一遍
