...
def build_signal_extraction_prompt(text: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "Only list entities used as fact subjects or genuine entity objects. Paths, version labels, document editions, hardware specifications and descriptions of tools are literal values, not separate projects.",
            "Use object_type=entity only for the exact name or id of a durable entity; a name with a version, count or descriptive sentence is a literal. Do not turn every mentioned noun into an entity.",
            "Confidence measures support in the text: direct, explicit statements normally score 0.85-1.0; partial or uncertain support scores lower. Do not reduce confidence merely for Chinese text or mundane technical details.",
            "Use status for independent progress observations. Use lifecycle_state only for mutually exclusive states such as active, completed or cancelled.",
