...
def valid_deepseek_config_text(root: Path) -> str:
    return f"""
[deepseek]
...
def test_load_config_valid_file(tmp_path: Path) -> None:
    ...
    assert config.deepseek is None
...
def test_load_config_deepseek_file_valid(tmp_path: Path) -> None:
    root = tmp_path / "brain"
    config_path = tmp_path / "config.toml"
    config_path.write_text(valid_deepseek_config_text(root), encoding="utf-8", newline="\n")
